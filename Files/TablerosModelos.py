...
def main():
    ...
    df = pd.read_csv('Statics/Data/df_final.csv',index_col=20)
    df_cluster = pd.read_csv('Statics/Data/DF_Final_Cluster.csv',index_col=2)
    ...
    with st.container():
        ...
        select_cols = st.columns([2, 2, 1, 4])  # Adjust column ratios as needed
        with select_cols[0]:
            linea = st.selectbox('Linea', options=['Todas', 'Pintado 1', 'Pintado 2', 'Cluster'])

        # Create an empty container for the 'Pintura' selectbox
        pintura_placeholder = select_cols[1].empty()

        # Conditionally render the 'Pintura' selectbox
        if linea != 'Cluster':
            with pintura_placeholder:
                pintura = st.selectbox("Pintura", options=pintura_options)
        else:
            pintura = None  # Set 'pintura' to None when 'Cluster' is selected

        with select_cols[2]:
            entrenamiento = st.text_input("Entrenamiento", "30")  # Default value is 30%
        with select_cols[3]:
            variable = st.selectbox('Variable', options=['Random Forest', 'Gradient Boosting Regressor'])
        ...
        if linea == "Todas":
            ...
            df_selected = df_top_pinturas[df_top_pinturas['Linea'] == linea].copy()

        if pintura is not None and pintura != "Todas":
